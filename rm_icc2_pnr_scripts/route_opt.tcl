...
#pk manual:
# open_block ${DESIGN_NAME}/${ROUTE_OPT_BLOCK_NAME}
...
if {[file exists [which $TCL_USER_ROUTE_OPT_SCRIPT]]} {
	...
	## If a config file is not provided, route_opt reverts to ICC-II's native extraction.
	...
	## StarRC in-design extraction (optional) : validation flow
	...
	## Without CLIB, route_opt issues TIM-260 and reverts to ICC-II's native timer, even if search_path includes .db paths. 
	...
	if {$ROUTE_OPT_ECO_OPT_PT_PATH == ""} {
		...
		## ECO fusion  
		#########################################
		## Prerequisites : (1) PT path (2) license (3) ROUTE_OPT_ECO_OPT_STARRC_CONFIG_FILE for StarRC-in-design extraction   
		...
		##        (2) StarRC-in-design extraction is OBD (extract.starrc_mode = true) which requires a valid ROUTE_OPT_ECO_OPT_STARRC_CONFIG_FILE.
		##            To use ICC-II native extraction, set extract.starrc_mode to false
		if {[check_license -quiet "Digital-AF"]} {
			if {[get_app_option_value -name extract.starrc_mode]} {
				## With StarRC-in-design extraction, it requires a configuration file
...
source rm_setup/panteaScripts/manual_drc_fixes.tcl
