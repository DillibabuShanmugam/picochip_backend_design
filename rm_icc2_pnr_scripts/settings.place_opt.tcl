...
## CCD WNS focus (optional)
## - By default CCD focuses on WNS per scenario. The following allows you to specify which path groups or end points
##   you want CCD to focus on addtionally. For example, you can specify ICG groups or memory groups requiring most attention 
##   so that CCD will focus on them additionally.
## - Affects compile_fusion (FC), place_opt and clock_opt build_clock. Enable the feature by specifying either or both of the following.
##   If both are specified, for endpoints, the focus will be limited to the end points of the specified path groups only
#	set_app_options -name ccd.targeted_ccd_path_groups -value "path_group_1 path_grup_2 ..."
#	set_app_options -name ccd.targeted_ccd_end_points_file -value "file_name"

## CCD WNS effort level - controls CCD engine effort on setup WNS optimization (optional)
## Higher effort may sacrifice TNS and power to achieve better WNS result. Applies to clock_opt final_opto and route_opt.
...
## Higher effort improves setup timing more at possible cost of power. Applies to CCD of all stages.
...
## ICG merging (Tool default is true; optional to set it to false)
...
##########################################################################################
## Additional Power related settings
##########################################################################################
...
if {$PREROUTE_CTS_POWER_AWARE_PRUNING != ""} {
...
## Dynamic power shaping (DPS) (optional)
## - Optimizes peak current and bulk dynamic voltage drop (DVD) by clock scheduling. Reduces DVD across the design.
##   Takes effect during place_opt final_opto.
##   Analyzes timing, power waveforms and optimizes clock latency offsets, then affects two areas:
##   1. Clock balance points to direct CTS to implement the offsets 
##   2. Timing network latencies to make pre-CTS steps see the impact.
## 
## - Typical usage:
##   1. Make sure all power and ground nets are connected. Run create_pg_net -automatic if unsure.
##   2. Enable DPS
#		set_app_options -name place_opt.flow.enable_dps -value true ;# enables DPS
#		set_app_options -name ccd.dps.flow_reporting -value {*} ;# generates DPS report at place_opt in a directory ./dps_date_number
##   3. Run place_opt (or place_opt final_opto if using breakppints) 
##   4. Run redhawk fusion after clock_opt/route_opt stage to verify dynamic voltage drop improvements
##      Refer to redhawk_in_design_pnr.tcl. You can set REDHAWK_IN_DESIGN_PNR_FROM_BLOCK_NAME to clock_opt or route_opt
if {$PLACE_OPT_ENABLE_DPS && [check_license -quiet "Digital-AF"]} {
	puts "RM-info: Setting place_opt.flow.enable_dps to true (tool default false)"
	set_app_options -name place_opt.flow.enable_dps -value true ;# enables DPS
	puts "RM-info: Setting ccd.dps.flow_reporting to * (tool default unspecified)"
	set_app_options -name ccd.dps.flow_reporting -value {*} ;# generates DPS report at place_opt in a directory ./dps_date_number
}

##########################################################################################
## Additional Placement and Congestion related settings
##########################################################################################
...
if {$PREROUTE_PLACEMENT_AUTO_DENSITY != ""} {
...
## - Selects max util based on the design stage as well as design technology
## - Potentially benefits design TNS
if {$PREROUTE_PLACEMENT_ENHANCED_AUTO_DENSITY != ""} {
...
if {$PREROUTE_PLACEMENT_GR_SOFT_CONGESTION_MAP != ""} {
	puts "RM-info: Setting route.global.export_soft_congestion_maps to $PREROUTE_PLACEMENT_GR_SOFT_CONGESTION_MAP (tool default false)"
	set_app_options -name route.global.export_soft_congestion_maps -value $PREROUTE_PLACEMENT_GR_SOFT_CONGESTION_MAP
}
...
if {$PLACE_OPT_CLOCK_AWARE_PLACEMENT != ""} {
...
##########################################################################################
## Additional CTS settings (all optional)
##########################################################################################
...
## The ICC-II timing and run time may be impacted. Only affects routed designs.
