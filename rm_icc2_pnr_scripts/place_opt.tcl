...
if {[file exists [which $TCL_USER_PLACE_OPT_SCRIPT]]} {
	...
	if {$PLACE_OPT_TRIAL_CTS != ""} {
		puts "RM-info: Setting place_opt.flow.trial_clock_tree to $PLACE_OPT_TRIAL_CTS (tool default false)"
		set_app_options -name place_opt.flow.trial_clock_tree -value $PLACE_OPT_TRIAL_CTS ;# default false
	}
	...
	if {$PLACE_OPT_OPTIMIZE_ICGS != ""} {
		puts "RM-info: Setting place_opt.flow.optimize_icgs to $PLACE_OPT_OPTIMIZE_ICGS (tool default false)"
		set_app_options -name place_opt.flow.optimize_icgs -value $PLACE_OPT_OPTIMIZE_ICGS ;# default false
		if {$PLACE_OPT_OPTIMIZE_ICGS_CRITICAL_RANGE != ""} {
			puts "RM-info: Setting place_opt.flow.optimize_icgs_critical_range to $PLACE_OPT_OPTIMIZE_ICGS_CRITICAL_RANGE (tool default 0.75)"
