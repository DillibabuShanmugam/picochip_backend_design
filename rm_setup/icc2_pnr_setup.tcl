...
##########################################################################################
set INIT_DESIGN_INPUT 		"DP_RM_NDM"	;# Specify one of the 3 options: ASCII | DC_ASCII | DP_RM_NDM; default is ASCII.
...
set PREROUTE_PLACEMENT_AUTO_DENSITY	"" ;# true|false; tool default true; optional in RM to set it to false if you want to disable the feature; 
...
set PREROUTE_PLACEMENT_ENHANCED_AUTO_DENSITY "" ;# false|true, tool default false; optional to set it to true in RM;
...
set PREROUTE_PLACEMENT_PIN_DENSITY_AWARE "" ;# false|true; tool default false; optional to set it to true in RM;
					;# sets app option place.coarse.pin_density_aware to control maximum local pin density;
set PREROUTE_PLACEMENT_GR_SOFT_CONGESTION_MAP "" ;# false|true; tool default false; optional to set it to true in RM;
					;# sets route.global.export_soft_congestion_maps to enable placer support for GR soft congestion map
set PREROUTE_NDR_OPTIMIZATION 		"" ;# false|true, tool default false; optional to set it to true in RM;
					;# sets place_opt/clock_opt.flow.optimize_ndrs to true enables NDR optimization

set PREROUTE_CTS_POWER_AWARE_PRUNING	"" ;# false|true; tool default false; optional to set it to true in RM;
...
set PLACE_OPT_USER_INSTANCE_NAME_PREFIX "OPT_" ;# Specify the prefix for new cells created by place_opt; default "" which means no user prefix
set TCL_USER_PLACE_OPT_PRE_SCRIPT 	"./rm_setup/panteaScripts/placement_customization.tcl" ;# An optional Tcl file for place_opt.tcl to be sourced before place_opt.
set TCL_USER_PLACE_OPT_SCRIPT 		"" ;# An optional Tcl file for place_opt.tcl to replace pre-existing place_opt commands.
set TCL_USER_PLACE_OPT_POST_SCRIPT 	"./rm_setup/panteaScripts/post_placement.tcl" ;# An optional Tcl file for place_opt.tcl to be sourced after place_opt.
...
set PLACE_OPT_TRIAL_CTS 		"" ;# false|true; tool default false; optional in RM;
...
set PLACE_OPT_OPTIMIZE_ICGS 		"" ;# false|true; tool default false; optional in RM;
...
set PLACE_OPT_MERGE_ICGS		"" ;# false|true; tool default true; optional in RM to set it to false;
...
set PLACE_OPT_ICG_AUTO_BOUND		"" ;# false|true; tool default false; optional to set it to true in RM;
					;# sets place.coarse.icg_auto_bound to enable use of automatically created group bounds
set PLACE_OPT_CLOCK_AWARE_PLACEMENT	"" ;# false|true; tool default false; optional to set it to true in RM;
...
set PLACE_OPT_MULTIBIT_BANKING 		"" ;# false|true; tool default false; optional in RM;
...
set PLACE_OPT_MULTIBIT_DEBANKING 	"" ;# false|true; tool default false; optional in RM;
...
set PLACE_OPT_ENABLE_DPS		false ;# false|true; tool default false; optional in RM;
					;# sets place_opt.flow.enable_dps to true and ccd.dps.flow_reporting to * to enable DPS (dynamic power shaping);
					;# optimizes peak current and bulk dynamic voltage drop (DVD) by clock scheduling. Reduces DVD across the design.
					;# takes effect in place_opt final_opto; see settings.place_opt.tcl for more details; requires Digital-AF license.
...
set CLOCK_OPT_CTS_ACTIVE_SCENARIO_LIST  "" ;# A subset of scenarios to be made active during clock_opt_cts step;
					;# once set, the list of active scenarios is saved and carried over to subsequent steps;
set CLOCK_OPT_CTS_USER_INSTANCE_NAME_PREFIX "CTS_" ;# Specify the prefix for new cells created by clock_opt build_clock; default "" which means no user prefix
set TCL_USER_CLOCK_OPT_CTS_PRE_SCRIPT 	"./rm_setup/panteaScripts/pre_cts.tcl" ;# An optional Tcl file for clock_opt_cts.tcl to be sourced before clock_opt.
...
set TCL_USER_CLOCK_OPT_OPTO_PRE_SCRIPT 	"./rm_setup/panteaScripts/pre_clock_opt.tcl" ;# An optional Tcl file for clock_opt_opto.tcl to be sourced before clock_opt.
...
set TCL_USER_CLOCK_OPT_OPTO_POST_SCRIPT "./rm_setup/panteaScripts/post_cts.tcl" ;# An optional Tcl file for clock_opt_opto.tcl to be sourced after clock_opt.
...
set PREROUTE_CTS_LOW_POWER_TECHNIQUE	"" ;# gate_relocation|low_power_targets|all; RM default is unspecified;
					...
					;#                    internal constraints and build clock trees with less area and power;
  					;# all: sets cts.compile.power_opt_mode to all to enable both the above features
set CLOCK_OPT_OPTO_CTO 			false ;# Default false; enables post-route clock tree optimization for non-CCD flow in clock_opt_opto.tcl
...
set TCL_USER_ROUTE_AUTO_PRE_SCRIPT 	"./rm_setup/panteaScripts/pre_route_auto.tcl" ;# An optional Tcl file for route_auto.tcl to be sourced before route_auto.
...
set ROUTE_AUTO_ANTENNA_FIXING		true;# false|true; tool default false; optional in RM; pk
...
#set TCL_ANTENNA_RULE_FILE		"./rm_setup/panteaScripts/antennaRule.tcl" ;# Antenna rule file; required if ROUTE_AUTO_ANTENNA_FIXING is set to true.
#set TCL_ANTENNA_RULE_FILE		"/home/pantea/018G_6lm4X1U_40KUTM_Syn/PR_tech/Synopsys/SCM/antennaRule_018_6lm.tcl" ;# Antenna rule file; required if ROUTE_AUTO_ANTENNA_FIXING is set to true.
set TCL_ANTENNA_RULE_FILE		"./rm_setup/panteaScripts/antennaRule_018_6lm.tcl" ;# Antenna rule file; required if ROUTE_AUTO_ANTENNA_FIXING is set to true.
...
set ROUTE_OPT_STARRC_CONFIG_FILE 	"" ;# Specify the configuration file for StarRC in-design extraction before the first route_opt in route_opt.tcl;
					;# required; refer to templates/route_opt.starrc_config_example.txt as an example
set ROUTE_OPT_PT_DELAY_CALCULATION_WITH_PBA false ;# Default false; sets time.pba_optimization_mode to path to enable PBA for second route_opt
...
set ROUTE_OPT_ECO_OPT_WITH_PBA 		"$ROUTE_OPT_PT_DELAY_CALCULATION_WITH_PBA" 
					;# Default is same as ROUTE_OPT_PT_DELAY_CALCULATION_WITH_PBA (which enables PBA for second route_opt; 
					;# sets time.pba_optimization_mode to path to enable PBA during eco_opt in the route_opt.tcl
...
set CHIP_FINISH_METAL_FILLER_PREFIX 	"filler_" ;# A string to specify the prefix for metal filler (decap) cells.
...
set CHIP_FINISH_METAL_FILLER_LIB_CELL_LIST "FILL1BWP7T FILL2BWP7T FILL4BWP7T FILL8BWP7T FILL16BWP7T FILL32BWP7T FILL64BWP7T" ;# A list of metal filler (decap) lib cells, including the library name, for ex, 
...
#set ICV_IN_DESIGN_DRC_CHECK_RUNSET 	"/opt/libraries/TSMCHOME/tsmc18_3v/Calibre/drc/calibre.drc" ;# The foundry runset for ICV used by signoff_check_drc
...
## write_gds related
#pk:
#set WRITE_GDS_LAYER_MAP_FILE 		"/opt/libraries/TSMCHOME/digital/Back_End/milkyway/tcb018gbwp7t_270a/gdsout.map" ;# A layer map file which provides a mapping between the tool and GDS layers
#set WRITE_GDS_LAYER_MAP_FILE 		"/home/pantea/018G_6lm4X1U_40KUTM_Syn/PR_tech/Synopsys/GdsOutMap/gdsout.map" ;# A layer map file which provides a mapping between the tool and GDS layers
# set WRITE_GDS_LAYER_MAP_FILE 		"/home/pantea/018G_6lm4X1U_40KUTM_Syn/gdsout.map" ;# A layer map file which provides a mapping between the tool and GDS layers

set WRITE_GDS_LAYER_MAP_FILE            "/home/dshanmugam/picochip-master/backend/picochip_library_dependencies/018G_6lm4X1U_40KUTM_Syn/gdsout.map" ;

#set WRITE_GDS_LAYER_MAP_FILE 		"/home/pkiaei/picochip_library_dependencies/018G_6lm4X1U_40KUTM_Syn/gdsout.map" ;# A layer map file which provides a mapping between the tool and GDS layers
#set WRITE_GDS_LAYER_MAP_FILE 		"/opt/libraries/TSMCHOME/digital/Back_End/milkyway/tpz973gv_280a/mt_2/6lm/gdsout.map" ;# A layer map file which provides a mapping between the tool and GDS layers
...
set search_path [list ./rm_icc2_pnr_scripts ./rm_setup ./templates $REDHAWK_SEARCH_PATH $front_end_path $cell_lib_path $io_lib_path $mem_path_best $mem_path_typ $mem_path_worst]
lappend search_path .
...
#set WRITE_DATA_FROM_BLOCK_NAME 		$CHIP_FINISH_BLOCK_NAME 	;# Label name of the source block in write_data.tcl;
set WRITE_DATA_BLOCK_NAME 		"picochip_postlayout" 			;# Label name to be used when saving a block in write_data.tcl
