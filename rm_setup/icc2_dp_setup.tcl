...
#set DP_BLOCK_REFS                     [list aspulsgfs1p16384x8cm16sw0] ;# design names for each physical block (including black boxes) in the design;
...
##########################################################################################
set TCL_TIMING_RULER_SETUP_FILE       "./rm_setup/panteaScripts/parasitic_constraints_file.tcl" ;# file sourced to define parasitic constraints for use with timing ruler 
                                          # before full extraction environment is defined
                                          # Example setup file:
                                          #       set_parasitic_parameters \
                                          #         -early_spec para_WORST \
                                          #         -late_spec para_WORST
set CONSTRAINT_MAPPING_FILE           "./rm_setup/panteaScripts/constraint_mapping_file" ;# Constraint Mapping File. Default is "split/mapfile"
...
##########################################################################################
set TCL_PHYSICAL_CONSTRAINTS_FILE     "./rm_setup/panteaScripts/physical_constraints.tcl" ;# TCL script for primary die area creation. If specified, DEF_FLOORPLAN_FILES will be loaded after TCL_PHYSICAL_CONSTRAINTS_FILE
...
# 				PLACE_IO
##########################################################################################
set TCL_PAD_CONSTRAINTS_FILE          "./rm_setup/panteaScripts/pad_constraints.tcl" ;# file sourced to create everything needed by place_io to complete IO placement
                                         ;# including flip chip bumps, and io constraints
set TCL_RDL_FILE                      "" ;# file sourced to create RDL routes
set TCL_USER_PLACE_IO_PRE_SCRIPT      "./rm_setup/panteaScripts/create_pad_cells.tcl" ;# An optional Tcl file to be sourced at the very beginning of the task
...
# 				PLACEMENT
##########################################################################################
set TCL_PLACEMENT_CONSTRAINTS_FILE    "./rm_setup/panteaScripts/placement_constraints.tcl" ;# Placeholder for any macro or standard cell placement constraints & options.
...
set CONGESTION_DRIVEN_PLACEMENT       "macro" ;# Set to one of the following: std_cell, macro, or both to enable congestion driven placement
set TIMING_DRIVEN_PLACEMENT           "" ;# Set to one of the following: std_cell, macro, or both to enable timing driven placement
set TCL_USER_PLACEMENT_PRE_SCRIPT     "./rm_setup/panteaScripts/fix_memory_macros.tcl" ;# An optional Tcl file to be sourced at the very beginning of the task
set TCL_USER_PLACEMENT_POST_SCRIPT    "./rm_setup/panteaScripts/post_dp_placement.tcl" ;# An optional Tcl file to be sourced at the very end of the task
...
# 				PNS
##########################################################################################
set TCL_PNS_FILE                      "./rm_setup/panteaScripts/pns.tcl" ;# File sourced to define all power structures. 
...
#set TCL_COMPILE_PG_FILE               "./rm_setup/panteaScripts/compile_pg.tcl" ;# File should contain all the compile_pg_* commands to create the power networks 
...
set TCL_USER_CREATE_POWER_PRE_SCRIPT  "./rm_setup/panteaScripts/connect_pg_nets.tcl" ;# An optional Tcl file to be sourced at the very beginning of the task
set TCL_USER_CREATE_POWER_POST_SCRIPT "./rm_setup/panteaScripts/pna.tcl" ;# An optional Tcl file to be sourced at the very end of the task
...
set mem_path_best "$memory_path/NDM/aspulsgfs1p8192x8cm16sw0/INPUT/Best/"
set mem_path_typ "$memory_path/NDM/aspulsgfs1p8192x8cm16sw0/INPUT/Typical/"
set mem_path_worst "$memory_path/NDM/aspulsgfs1p8192x8cm16sw0/INPUT/Worst/"

set search_path [list ./rm_icc2_dp_scripts ./rm_icc2_pnr_scripts $WORK_DIR $front_end_path $cell_lib_path $io_lib_path $mem_path_best $mem_path_typ $mem_path_worst ./rm_setup/panteaScripts/upf_files/] 
lappend search_path .
